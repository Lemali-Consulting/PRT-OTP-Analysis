...
def analyze(df: pl.DataFrame) -> tuple[pl.DataFrame, float]:
    """Compute Pearson correlation between stop count and OTP."""
    r = df.select(pl.corr("stop_count", "avg_otp")).item()
    return df, r


def make_chart(df: pl.DataFrame, r: float) -> None:
    """Generate scatter plot of stop count vs OTP."""
    plt = setup_plotting()
