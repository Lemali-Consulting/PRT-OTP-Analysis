...
- **[convention]**: Two stop-id namespaces exist. GTFS files (`stops.txt`, and analyses that read them, e.g. 53) use numeric `stop_id` (e.g. `22999`). The `prt.db` `stops`/`route_stops`/`stop_signals` tables use PRT internal alpha-prefixed codes (e.g. `E08092`). They do **not** join on `stop_id` — joining silently yields zero matches. Bridge via `stop_code` (shared by both, e.g. `22778`). Use `stop_id` only when joining within the prt.db internal tables.
- **[convention]**: In pipeline `SOURCES.yaml`, *consumed* `tables:` are bare strings (`- stops`); only `tables_produced:` uses dict-format (`- name:`/`description:`). A dict in `tables:` crashes the website build (`page_sources`: "cannot use 'dict' as a dict key"). Analyses' `tables:` are bare strings too.